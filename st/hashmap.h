...
size_t hashmap_hash_uint32(const void *key) { return *(uint32_t *)key; }

int hashmap_compare_uint32(const void *a, const void *b) { return *(int32_t *)a - *(int32_t *)b; }
...
size_t hashmap_table_min_size_calc(size_t num_entries) { return num_entries + (num_entries / 3); }
...
int hashmap_init(struct hashmap *map, hashmap_hash_func_t hash_func, hashmap_key_compare_func_t key_compare_func,
                 size_t initial_size)
{
    assert(map != NULL);
...
size_t hashmap_calc_index(const struct hashmap *map, const void *key) { return HASHMAP_SIZE_MOD(map, map->hash(key)); }

struct hashmap_entry *hashmap_entry_find(const struct hashmap *map, const void *key, bool find_empty)
{
