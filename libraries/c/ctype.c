...
int isascii(int ch) { return (unsigned)ch <= 0x7F; }

int iscntrl(int ch) { return 0 <= ch && ch < 32; }
...
int ispunct(int ch) { return isprint(ch) && ch != ' ' && !isalnum(ch); }
...
int toascii(int ch) { return (unsigned)ch & 0x7F; }

int tolower(int ch) { return ch + 0x20 * ((ch >= 'A') && (ch <= 'Z')); }

int toupper(int ch) { return ch - 0x20 * ((ch >= 'a') && (ch <= 'z')); }
